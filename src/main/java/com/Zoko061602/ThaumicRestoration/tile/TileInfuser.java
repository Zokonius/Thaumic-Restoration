...
public class TileInfuser extends TileThaumcraftInventory {
	   ...
	   if(rotation>=360){
		 ...
		 rounds++;
		  ...
		  if(getPedestals().get(rounds-1).getStackInSlot(0).getItem() instanceof ItemCrystalEssence)
			if(AspectHelper.getObjectAspects(getPedestals().get(rounds-1).getStackInSlot(0)).getAspects()[0]==recipe.getAspect()) {
	...
	private Aspect checkItems(){
		...
		for(TilePedestal p:getPedestals())
         if(!(p.getStackInSlot(0).isEmpty()) && p.getStackInSlot(0).getItem() instanceof ItemCrystalEssence){
